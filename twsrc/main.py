...
def getAccounts(path: str = ACCOUNTS_PATH):
    ...
    with open(path, 'r') as file:
        for line in file:
            line = line.strip()
            yield line.split('----')[: 5]  # [用户名, 用户密码, 邮箱地址, 邮箱密码, auth_token]
...
async def main():
    ...
    accountsCount = sum(1 for _ in getAccounts())
    ...
    #         proxy = None if proxiesCount == 0 else PROXIES[id % proxiesCount]
    ...
    with Writer(OUTPUT_DIR) as writer:
        for i, keyword in enumerate(getKeywords(), start = 1):
            ...
            epsTime = time.time()
            info(
                f'starting download for keyword "{keyword}"'
            )
